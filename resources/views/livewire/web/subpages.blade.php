<div>
    @section('title')
        {{ __('web/subpages.title') }}
    @endsection
    <x-layout.page-title>
        <div></div>
        <div class="flex space-x-3">
            <a href="{{ url('web/subpage/create') }}" class="btn-primary">
                            ...
                            @foreach ($subpages as $key => $subpage)
                                <tr>
                                    <td class="py-3 pl-6 whitespace-nowrap">
                                        {{ $subpages->firstItem()+$key }}
                                    </td>
                                    <td class="w-full px-4 py-3 whitespace-nowrap">
                                        <a href="{{ url('web/subpage/edit/' . $subpage->id) }}" class="text-sm text-left text-gray-900 hover:text-light-blue-500">
                                            <div>
                                                {{ $subpage->title }}
                                            </div>
                                    ...
                                    <td class="px-4 py-3 text-sm text-right text-gray-500 whitespace-nowrap">
                                        {{ date("H:i d.m.Y", strtotime($subpage->created_at)) }} 
                                        <span class="text-gray-400">/</span> 
                                        {{ date("H:i d.m.Y", strtotime($subpage->updated_at)) }}
                                    </td>
                                    <td class="py-3 pr-6 text-sm font-medium text-right whitespace-nowrap">
                                        <div class="flex space-x-2">
                                            ...
                                            <a href="{{ url('web/subpage/edit/' . $subpage->id) }}" class="text-light-blue-400 hover:text-light-blue-500">
                                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
