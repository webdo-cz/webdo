    ...
    <div class="mb-4 bg-white">
        @foreach($articles as $key => $article)
            <div class="flex {{ $loop->first ? '' : 'border-t' }}">
                <a href="{{ url('web/articles/edit-record/' . $article->id) }}" class="flex flex-col justify-between flex-grow w-2/3 px-4 py-4 text-sm font-medium leading-5 transition duration-200 cursor-pointer md:items-center md:flex-row sm:px-6 hover:bg-light-blue-500 hover:text-white">
                    <div class="flex items-center">
                        ...
                        <div class="truncate">
                            {{ $article->title }}<br>
                            <span class="opacity-60">Autor: {{ $article->user->name }}</span>
                        </div>
                    </div>
                    <div class="flex flex-col flex-shrink-0 sm:ml-2 sm:flex-row sm:items-center sm:space-x-4">
                        <div class="mb-2 text-xs sm:text-right sm:mb-0 xs:text-sm">
                            <span class="hidden opacity-60 md:inline-block">Vytvořeno:</span> {{ date("H:i d.m.Y", strtotime($article->created_at)) }} <br class="hidden md:block">
                            @if ($article->updated_at != $article->created_at)
                            <span class="opacity-60 md:hidden">/</span>
                            <span class="hidden opacity-60 md:inline-block">Poslední úprava:</span> {{ date("H:i d.m.Y", strtotime($article->updated_at)) }}
                            @endif
                        </div>
                    </div>
                </a>
