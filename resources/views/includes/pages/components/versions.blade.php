<div x-show="open == '{{ $component->id }}'" class="px-6 pt-2 bg-white border-t" style="display: none;">
    @foreach($component->contentVersions as $variant)
        <div class="flex items-center justify-between py-3 {{ $loop->first ? '' : 'border-t' }}">
            <div class="text-sm text-gray-400">
                <a 
                    href="{{ url('content/' . $component->id . '/' . $variant->id) }}"
                ...
                >
                 ...
                 vytvořena: <span class="text-gray-600">{{ date("H:i d.m.Y", strtotime($component->created_at)) }}</span>
                @if ($component->updated_at != $component->created_at)
                , poslední úprava: <span class="text-gray-600">{{ date("H:i d.m.Y", strtotime($component->updated_at)) }}</span>
                @endif
            </div>
